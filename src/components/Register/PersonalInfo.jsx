  ...
  const onClickCheck = (type) => {
    if (type == 'email') {
      axios.get(`/auth/check/email/${registerInfo.email}`).then((res) =>
        setCheck((p) => ({
          ...p,
          email: res.data.isExisted ? '이미 가입된 이메일입니다.' : '사용 가능',
        })),
      );
    }
    if (type == 'nickName') {
  ...
  const checkedValidNick = () => {
    ...
    !valNick.test(registerInfo.nickName)
      ...
      : axios.get(`/auth/check/nickName/${registerInfo.nickName}`).then((res) =>
          setCheck((p) => ({
            ...p,
            nickName: res.data.isExisted ? '이미 사용중입니다.' : '사용 가능',
          })),
        );
  };
  return (
    <>
      ...
      <InfoWrapper className="col-container">
        <Wrapper className="row-container" checking={check.email == '사용 가능' && 'email'}>
          ...
          />
          <GreenBorderBtn type="button" onClick={() => onClickCheck('email')}>
            중복확인
          </GreenBorderBtn>
        </Wrapper>
        ...
        <Wrapper className="row-container" checking={check.nickName == '사용 가능' && 'nickName'}>
          ...
          <input
            ...
            maxLength={20}
          />
          <GreenBorderBtn type="button" onClick={() => onClickCheck('nickName')}>
            중복확인
          </GreenBorderBtn>
        </Wrapper>
