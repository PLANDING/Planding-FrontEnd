...
const StepContainer = ({ curriculum, setCurriculum, projectId, memberCnt }) => {
    ...
    const onClickDone = (event) => {
        ...
        if (id == 21) {
            if (window.confirm("프로젝트가 마감됩니다. \n프로젝트 마감후, 취소가 불가합니다. \n프로젝트 완료하시겠습니까?")) {
                axios.patch(`/myProject/complete/${projectId}`)
                    .then(res => {
    ...
    const currArr = [
        ...
        [{ content: "프로젝트 코드 리팩토링", curr: 18 }, { content: "프로젝트 테스트", curr: 19 }, { content: "프로젝트 배포", curr: 20 }, { content: "프로젝트 완료", curr: 21 }],
    ];
                        ...
                        <DropDown>
                            {curr.map(arr =>
                                <CurriBox className="row-container">
                                    ...
                                    <img id={arr.curr} src={curriculum >= arr.curr ? doneFillImg : doneImg} onClick={curriculum >= arr.curr ? onClickDel : onClickDone} width="24px" />
                                </CurriBox>)}
                        </DropDown>}
                </StepBox>)}
        </Container>
